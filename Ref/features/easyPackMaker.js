(function(config, GM, utils) {
    ...
    function injectCalculator(targetElement, formulaType) {
        ...
        container.style.cssText = 'margin-top:20px;padding:15px;border:1px solid #ddd;border-radius:8px;background-color:#f9f9f9;box-sizing:border-box;';

        const typeInfo = document.createElement('div');
        typeInfo.style.cssText = 'margin-bottom:15px;font-weight:bold;color:#333;font-size:14px;';
        typeInfo.textContent = `📊 Тип: ${typeName}`;
        container.appendChild(typeInfo);

        const mainContent = document.createElement('div');
        mainContent.style.cssText = 'display:flex;gap:20px;align-items:flex-start;flex-wrap:nowrap;';

        const inputsColumn = document.createElement('div');
        inputsColumn.style.cssText = 'flex:0 0 180px;min-width:180px;';
        inputsColumn.innerHTML = `
            <div style="margin-bottom:10px;"><label style="display:block;font-weight:600;font-size:13px;color:#444;margin-bottom:4px;white-space:nowrap;">Ширина L, мм</label><input type="number" id="tm-input-l" style="width:100%;padding:6px 8px;box-sizing:border-box;border:1px solid #ccc;border-radius:3px;font-size:13px;" placeholder="0" min="1" step="1"></div>
            <div style="margin-bottom:10px;"><label style="display:block;font-weight:600;font-size:13px;color:#444;margin-bottom:4px;white-space:nowrap;">Глубина W, мм</label><input type="number" id="tm-input-w" style="width:100%;padding:6px 8px;box-sizing:border-box;border:1px solid #ccc;border-radius:3px;font-size:13px;" placeholder="0" min="1" step="1"></div>
            <div style="margin-bottom:10px;"><label style="display:block;font-weight:600;font-size:13px;color:#444;margin-bottom:4px;white-space:nowrap;">Высота H, мм</label><input type="number" id="tm-input-h" style="width:100%;padding:6px 8px;box-sizing:border-box;border:1px solid #ccc;border-radius:3px;font-size:13px;" placeholder="0" min="1" step="1"></div>
        `;

        const rightColumn = document.createElement('div');
        rightColumn.style.cssText = 'flex:1;min-width:280px;display:flex;flex-direction:column;align-items:center;gap:10px;';
        rightColumn.innerHTML = `
            <div style="width:100%;text-align:center;"><img src="${imageUrl}" alt="Scheme" style="max-width:100%;height:auto;max-height:200px;" onerror="this.src='https://via.placeholder.com/300x200?text=Image+Not+Found'"></div>
            <div id="tm-results" style="display:none;width:50%;padding:8px 12px;background:#e8f5e9;border:2px solid #4CAF50;border-radius:6px;box-sizing:border-box;">
                <div style="display:flex;justify-content:space-between;gap:15px;">
                    <div style="flex:1;min-width:0;"><div style="font-size:11px;color:#2e7d32;font-weight:600;margin-bottom:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">📐 Ширина:</div><div style="font-size:15px;color:#1b5e20;font-weight:bold;white-space:nowrap;"><span id="tm-result-width">0</span> <span style="font-size:12px;font-weight:normal;">мм</span></div></div>
                    <div style="width:1px;background:#81c784;"></div>
                    <div style="flex:1;min-width:0;"><div style="font-size:11px;color:#2e7d32;font-weight:600;margin-bottom:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">📏 Высота:</div><div style="font-size:15px;color:#1b5e20;font-weight:bold;white-space:nowrap;"><span id="tm-result-height">0</span> <span style="font-size:12px;font-weight:normal;">мм</span></div></div>
                </div>
            </div>
        `;
        ...
        availabilityDiv.style.cssText = 'display:none;margin-top:15px;padding:12px 15px;background:#fff3e0;border:2px solid #ff9800;border-radius:6px;box-sizing:border-box;font-size:13px;';
        availabilityDiv.innerHTML = `
            <div style="display:flex;gap:30px;margin-bottom:8px;"><div style="flex:1;min-width:200px;"><div style="font-weight:700;color:#e65100;font-size:13px;display:flex;align-items:center;gap:6px;">🖨️ Доступно для печати:</div></div><div style="flex:1;min-width:200px;"><div style="font-weight:700;color:#e65100;font-size:13px;display:flex;align-items:center;gap:6px;">✂️ Доступно для изготовления на:</div></div></div>
            <div style="display:flex;gap:30px;margin-bottom:12px;"><div style="flex:1;min-width:200px;"><div id="tm-print-methods" style="color:#bf360c;line-height:1.8;"></div></div><div style="flex:1;min-width:200px;"><div id="tm-production-methods" style="color:#bf360c;line-height:1.8;"></div></div></div>
            <div id="tm-unavailable-message" style="display:none;color:#c62828;font-weight:600;font-size:14px;padding:10px 0;border-top:1px solid #ffe0b2;margin-top:8px;">✗ Недоступно для изготовления. Можете сделать рассчёт в калькуляторе гофрокоробок.</div>
            <div id="tm-max-sizes-inline" style="display:none;margin-top:12px;padding-top:12px;border-top:1px solid #ffe0b2;">
                <div style="font-weight:700;color:#f57f17;margin-bottom:10px;font-size:12px;display:flex;align-items:center;gap:6px;">⚠️ Максимально доступные размеры:</div>
                <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:15px;">
                    <div style="padding:10px;background:#fff;border:1px solid #ffe0b2;border-radius:4px;"><div style="color:#f57f17;margin-bottom:5px;font-weight:600;font-size:11px;">L и W неизменны:</div><div style="color:#e65100;font-weight:bold;font-size:13px;"><span id="tm-size-l-1">0</span> × <span id="tm-size-w-1">0</span> × <span id="tm-size-h-1">0</span> <span style="font-size:11px;font-weight:normal;">мм</span></div></div>
                    <div style="padding:10px;background:#fff;border:1px solid #ffe0b2;border-radius:4px;"><div style="color:#f57f17;margin-bottom:5px;font-weight:600;font-size:11px;">L и H неизменны:</div><div style="color:#e65100;font-weight:bold;font-size:13px;"><span id="tm-size-l-2">0</span> × <span id="tm-size-w-2">0</span> × <span id="tm-size-h-2">0</span> <span style="font-size:11px;font-weight:normal;">мм</span></div></div>
                    <div style="padding:10px;background:#fff;border:1px solid #ffe0b2;border-radius:4px;"><div style="color:#f57f17;margin-bottom:5px;font-weight:600;font-size:11px;">W и H неизменны:</div><div style="color:#e65100;font-weight:bold;font-size:13px;"><span id="tm-size-l-3">0</span> × <span id="tm-size-w-3">0</span> × <span id="tm-size-h-3">0</span> <span style="font-size:11px;font-weight:normal;">мм</span></div></div>
                </div>
            </div>
        `;
        ...
        mgiDiv.id = 'tm-mgi';
        mgiDiv.style.cssText = 'display:none;margin-top:15px;padding:10px 15px;background:#f3e5f5;border:2px solid #9c27b0;border-radius:6px;box-sizing:border-box;font-size:13px;';
        mgiDiv.innerHTML = `<div style="display:flex;align-items:center;gap:10px;"><span style="font-weight:700;color:#6a1b9a;font-size:13px;">✨ Доступность для нанесения MGI:</span><span id="tm-mgi-status" style="font-weight:600;font-size:14px;"></span></div>`;
        container.appendChild(mgiDiv);

        const costDiv = document.createElement('div');
        costDiv.id = 'tm-cost';
        costDiv.style.cssText = 'display:none;margin-top:15px;padding:12px 15px;background:#e3f2fd;border:2px solid #2196f3;border-radius:6px;box-sizing:border-box;font-size:13px;';
        costDiv.innerHTML = `
            <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:20px;">
                <div><div style="font-weight:700;color:#1565c0;margin-bottom:6px;font-size:13px;display:flex;align-items:center;gap:6px;">📝 Стоимость разработки штампа:</div><div style="color:#0d47a1;font-size:16px;font-weight:bold;">500 ₽</div></div>
                <div><div style="font-weight:700;color:#1565c0;margin-bottom:6px;font-size:13px;display:flex;align-items:center;gap:6px;">🔧 Стоимость изготовления штанцформы (1 коробка):</div><div id="tm-die-cost" style="color:#0d47a1;font-size:16px;font-weight:bold;">—</div></div>
                <div><div style="font-weight:700;color:#1565c0;margin-bottom:6px;font-size:13px;display:flex;align-items:center;gap:6px;">🎨 Адаптация дизайна клиента:</div><div style="color:#0d47a1;font-size:14px;font-style:italic;">по прайсу дизайнеров</div></div>
            </div>
        `;
        ...
        calcBtn.style.cssText = 'width:100%;margin-top:15px;padding:10px 15px;font-size:14px;font-weight:bold;cursor:pointer;background:#4CAF50;color:white;border:none;border-radius:4px;box-sizing:border-box;transition:background 0.2s;';
        ...
        ['tm-input-l', 'tm-input-w', 'tm-input-h'].forEach(id => {
            ...
            input.pattern = '\\d*';
            input.addEventListener('input', function() { this.value = this.value.replace(/[^\d]/g, '').replace(/^0+/, ''); });
        ...
        function fitsWithRotation(rectWidth, rectHeight, maxWidth, maxHeight) {
            return (rectWidth <= maxWidth && rectHeight <= maxHeight) || (rectWidth <= maxHeight && rectHeight <= maxWidth);
        }

        function findMaxValueForPrint(fixedL, fixedW, fixedH, variableType, formulas) {
            const step = 1, maxIterations = 2000;
            let maxValue = 0;
            for (let i = step; i <= maxIterations; i += step) {
                ...
                if (formulas.formula1) { try { const expr1 = formulas.formula1.replace(/\bL\b/g, L).replace(/\bW\b/g, W).replace(/\bH\b/g, H); heightResult = eval(expr1); } catch (e) { continue; } }
                if (formulas.formula2) { try { const expr2 = formulas.formula2.replace(/\bL\b/g, L).replace(/\bW\b/g, W).replace(/\bH\b/g, H); widthResult = eval(expr2); } catch (e) { continue; } }
                if (fitsWithRotation(widthResult, heightResult, 318, 476) || fitsWithRotation(widthResult, heightResult, 690, 478)) { maxValue = i; } else { break; }
            }
        ...
        function calculateDieCost(width, height, formulas) {
            ...
            return Math.ceil(finalCost / 1000);
        }
        ...
        calcBtn.addEventListener('click', () => {
            ...
            let widthResult = 0, heightResult = 0;
            if (formulas.formula1) { try { const expr1 = formulas.formula1.replace(/\bL\b/g, L).replace(/\bW\b/g, W).replace(/\bH\b/g, H); heightResult = eval(expr1); } catch (e) {} }
            if (formulas.formula2) { try { const expr2 = formulas.formula2.replace(/\bL\b/g, L).replace(/\bW\b/g, W).replace(/\bH\b/g, H); widthResult = eval(expr2); } catch (e) {} }
            ...
            const printOptions = [], productionOptions = [];
            ...
            if (digitalAvailable) printOptions.push('<span style="color:#2e7d32;font-weight:600;">✓ Цифра</span>');
            if (offsetAvailable) printOptions.push('<span style="color:#2e7d32;font-weight:600;">✓ Офсет</span>');

            const tigelAvailable = fitsWithRotation(widthResult, heightResult, 495, 730);
            if (tigelAvailable) productionOptions.push('<span style="color:#2e7d32;font-weight:600;">✓ Тигель</span>');

            const digitalPlotterAvailable = digitalAvailable && fitsWithRotation(widthResult, heightResult, 290, 448);
            if (digitalPlotterAvailable) productionOptions.push('<span style="color:#2e7d32;font-weight:600;">✓ Плоттер (цифра)</span>');

            const offsetPlotterAvailable = offsetAvailable && fitsWithRotation(widthResult, heightResult, 660, 460);
            if (offsetPlotterAvailable) productionOptions.push('<span style="color:#2e7d32;font-weight:600;">✓ Плоттер (офсет)</span>');
    ...
    function checkAndInject() {
        ...
        if (currentHeaderContent === lastHeaderContent && currentType) return;
    ...
    function init() {
        ...
        const setupObserver = () => {
            ...
            observer.observe(rootElement, { childList: true, subtree: true, characterData: true, attributes: true, attributeFilter: ['class', 'style'] });
            return true;
        };

        if (!setupObserver()) {
            const retryObserver = setInterval(() => {
                if (setupObserver()) clearInterval(retryObserver);
            }, 1000);
        }

        pollIntervalId = setInterval(() => {
            if (formulasData) checkAndInject();
        }, POLL_INTERVAL);
